...
def create_train_test(df: pd.DataFrame) -> [pd.DataFrame, pd.DataFrame, np.ndarray, np.ndarray]:
    """
    ...
    X_train = train.drop(columns=["label", "Outcome", "Outcome linked to object of search",
                                  "Removal of more than just outer clothing"])
    y_train = train[["label"]]

    # Test set
    X_test = test.drop(columns=["label", "Outcome", "Outcome linked to object of search",
                                "Removal of more than just outer clothing"])
    y_test = test[["label"]]
